...
from models import User, Publication, Grant
...
def test_publication_creation(activity: Dict, user_id: str) -> Optional[Publication]:
    ...
    try:
        pub = Publication(
            ...
            status_year=str(status_info.get('year')) if status_info.get('year') else None,
        )
...
def test_grant_creation(activity: Dict, user_id: str) -> Optional[Grant]:
    ...
    try:
        grant = Grant(
            ...
            total_funding=funding_info.get('fundedamount') or fields.get('Total Funding'),
...
def debug_user():
    # Load environment
    load_dotenv()
    database_url = os.getenv("DATABASE_URL", "postgresql://admin:secret@localhost/research")
    ...
    try:
        ...
        user = session.query(User).filter(User.id == '109546797').first()
        if not user:
        ...
        existing_pubs = session.query(Publication).filter(Publication.user_id == user.id).count()
        ...
        raw_response = api._make_request("/activities/-21", f"?data=detailed&userlist={user.id}")
                ...
                if isinstance(data_in_key, list):
                    ...
                    if len(data_in_key) > 0:
                        ...
                        if isinstance(data_in_key[0], dict):
                            print(f"  First item keys: {list(data_in_key[0].keys())[:15]}")
                            if 'fields' in data_in_key[0]:
                                fields = data_in_key[0]['fields']
                                print(f"  First item field keys: {list(fields.keys())[:15]}")
                                if 'Type' in fields:
