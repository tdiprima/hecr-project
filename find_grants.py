                    ...
                    if isinstance(value, list) and len(value) > 0:
                        ...
                        for item in value[:3]:
                            if isinstance(item, dict) and 'fields' in item:
                                ...
                                if 'Grant ID / Contract ID' in fields or 'Sponsor' in fields:
                                    if key not in grants_found:
                                        grants_found[key] = []
                                    grants_found[key].append(item)
                                    
                                    print(f"    ✓ GRANT FOUND in key '{key}'!")
                                    print(f"      Title: {fields.get('Title', 'N/A')[:60]}")
                                    print(f"      Grant ID: {fields.get('Grant ID / Contract ID', 'N/A')}")
                                    print(f"      Sponsor: {fields.get('Sponsor', 'N/A')}")
                                
                                # Also show what types are in this endpoint
                                if 'Type' in fields and item == value[0]:  # Only for first item
                                    print(f"    First item type in '{key}': {fields['Type']}")
